...
class SearchEngine:
    ...
    def __init__(self, vector_store, enable_ai_overview=False):
        ...
        # Initialize AI query expander for intelligent query enhancement
        try:
            from ai_query_expander import AIQueryExpander
            self.ai_expander = AIQueryExpander()
            self.query_cache = {}  # Cache expansions for performance
    ...
    def search(self, query: str, max_results: int = 30, include_ai_overview: bool = True) -> Dict:
        ...
        # Very relaxed evidence gate: retrieve broadly, let AI filter intelligently
        MIN_TOP_SCORE = 0.25
    ...
    def _enhance_query(self, query: str, analysis: Dict) -> str:
        """AI-powered query enhancement using Malta-specific legal terminology.

        This method uses AI to generate domain-specific terminology and document hints,
        making it universal across all legal domains (tax, corporate, notarial, etc.)
        without requiring hardcoded mappings.
        """

        # Try AI-powered expansion first
        if self.ai_expander:
            ...
            try:
                # Use AI to generate Malta-specific legal terminology
                enhanced_query = self.ai_expander.generate_enhanced_query(query)

                # Cache the result for future use
                self.query_cache[cache_key] = enhanced_query

                self.debug.log("info", f"AI expansion: {len(enhanced_query)} chars generated")
        ...
        # Fallback: Basic intent-based enhancement (used if AI fails)
        enhancements = {
